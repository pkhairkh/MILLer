...
/// Delegate to the lab crate's `StoreKnowledgeQuery`.
struct StoreKnowledgeQuery<'a> {
    inner: ane_lab::session::StoreKnowledgeQuery<'a>,
}

impl<'a> StoreKnowledgeQuery<'a> {
    fn new(store: &'a ane_knowledge::store::KnowledgeStore) -> Self {
        Self { inner: ane_lab::session::StoreKnowledgeQuery::new(store) }
    }
}
...
impl<'a> ane_passes::knowledge_query::PassKnowledgeQuery for StoreKnowledgeQuery<'a> {
    fn query_legality(
        ...
        self.inner.query_legality(op_pattern, scope)
    }
    ...
    fn query_risk(
        ...
        self.inner.query_risk(op_pattern, scope)
    }
    ...
    fn query_precision_hazard(
        ...
        self.inner.query_precision_hazard(op_pattern, current_dtype, scope)
    }
    ...
    fn query_compute_plan_placement(
        ...
        self.inner.query_compute_plan_placement(op_pattern, scope)
    }
}
...
fn run_lab(
    ...
    let session = ane_lab::session::LabSession {
...
fn run_lab_loop(
    ...
    let session = ane_lab::session::LabLoopSession {
        input: input.to_string(),
        output: output.to_string(),
        bridge_script: bridge_script.to_string(),
        python_path: python_path.to_string(),
...
/// Delegate to the lab crate's `build_artifact_manifest`.
fn build_artifact_manifest(
    ...
    ane_lab::session::build_artifact_manifest(spec, bridge_result, task_hash, compiler_version)
}

/// Build a backend-knowledge update from the compilation result.
///
/// Includes the deterministic task hash for identity tracking.
/// Delegate to the lab crate's `build_knowledge_update`.
fn build_knowledge_update(
    spec: &ane_ir::task_spec::SyntheticTaskSpec,
    bridge_result: &ane_bridge::subprocess::BridgeResult,
    task_hash: &str,
) -> serde_json::Value {
    ane_lab::session::build_knowledge_update(spec, bridge_result, task_hash)
}
