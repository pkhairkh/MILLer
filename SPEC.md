...
The `pkhairkh/qwen3-coreml-palettized` project demonstrates that a 1.7B-parameter transformer can survive ANE constraints by decomposing into five coordinated Core ML MLPrograms: `io_model` (embedding + LM head on CPU+GPU), three decoder shards (left/mid/right on CPU+NE), and a `sampler_model` (CPU+GPU). Each decoder shard owns its own KV state, uses a reverse ring-buffer cache, and applies mixed-precision grouped LUT palettization (4/6/8-bit varying by layer depth and weight type). This is not a packaging artifact. It is evidence of **one** backend-constrained execution form: shard-aware, state-aware, package-aware, and ANE-survivable. The near-perfect perplexity parity (delta < 0.6 on wikitext-103) proves this particular decomposition can preserve quality for this model. However, this five-package layout is not a universal prescription. Different model architectures, different parameter counts, and different target ANE families will produce different optimal decompositions. A 0.6B model may fit in fewer packages; a 7B model may need more. A model without GQA may not need the same shard boundaries. The compiler must **discover** the right decomposition per model by reasoning about actual ANE constraints (constant budget, state budget, op legality, bandwidth limits) rather than hardcoding any particular topology. Our system must generalize from this evidence — not by replicating it verbatim, but by capturing the constraint-solving principles it embodies and applying them dynamically to each new model.
...
**The Qwen3 shard layout is one validated instance of a backend-constrained execution form, not a universal template.**

The five-package decomposition (io_model + 3 decoder shards + sampler) is **one validated instance** of a backend-constrained execution form for Qwen3-1.7B, not a universal template. It exists because a single monolithic Qwen3-1.7B graph cannot survive ANE placement intact. The shard boundaries are not arbitrary: each shard must fit within ANE state and constant budgets, each owns its own KV state to avoid cross-package state coupling, and the io_model/sampler are on CPU+GPU because embedding, logit projection, and sampling contain operations that are ANE-hostile or ANE-suboptimal. The mixed-precision palettization (4/6/8-bit varying by layer and weight type) is not random: deeper layers and attention projections get higher precision because sensitivity analysis showed they need it.

However, other models will have different optimal decompositions. A smaller model (e.g., 0.6B) may require fewer shards; a larger model (e.g., 7B) may require more. A model with a different attention mechanism, different hidden dimensions, or a different target ANE family will produce different constraint solutions. The Qwen3-1.7B five-package form should be treated as **reference evidence** — a known-good point in the decomposition space — not as the only point.

The system must capture the **constraint-solving principles** (per-shard constant/state budget, op legality per compute unit, cross-shard handoff cost) rather than hardcode any particular package topology. The shard planner must be constraint-driven: given a model's SIR and a target ANE family's hardware limits, it must compute a viable decomposition. The Qwen3-1.7B form serves as a validation target (the planner should be able to reproduce it when given the same inputs) and as a seed for the shard template knowledge store, but it must not be the only decomposition the system can produce.
...
2. **The constraint-driven shard planner** (Phase 2). The system must be able to reproduce the Qwen3-1.7B five-package decomposition as a validation target (proving it can reason about real-world evidence), but it must also discover valid decompositions for other models dynamically. If the planner can only replicate one hardcoded topology, the system is wrong at a fundamental level.
