...
fn mir_op_to_apple_ops(
    ...
    match op {
        ...
        mir_compat::MirOpCompat::Fill { name, shape, value, dtype } => {
            // `fill` is ANE-illegal (no ANE converter; listed in CPU_ONLY ops).
            // Decompose to ANE-legal ops using the same pattern as FillLike:
            //   fill(shape, val) → const(zeros_of_shape) + broadcast(val)
            //
            // However, since we have the explicit shape, we can emit a more
            // efficient decomposition:
            //   Step 1: Create a zero tensor via mul(identity_shape, 0)
            //   Step 2: Add the fill value via add(zero_tensor, val)
            //
            // Since fill is never actively generated by any decomposition
            // (all FillLike usages have been replaced with Const+Add broadcasting),
            // this path is primarily defensive. If fill IS needed, the decomposition
            // produces a `const` op with a precomputed zero tensor + `add` for the value.
            //
            // Defensive fallback: emit as raw fill (will force CPU fallback).
            // This preserves backward compatibility in case Fill is produced
            // by external SIR graphs.
            let apple_dtype = mil_dtype_to_apple(dtype);
