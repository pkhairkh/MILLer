...
mod tests {
    ...
    use ane_ir::sir::{
        QualityContract, SirMetadata, SirNode, SirNodeId, SirOp, TaskOrigin,
    };
    ...
    fn assert_graphs_identical(left: &SirGraph, right: &SirGraph) {
        assert_eq!(left.nodes.len(), right.nodes.len(),
            "Node count mismatch: left={}, right={}", left.nodes.len(), right.nodes.len());
        assert_eq!(left.inputs, right.inputs,
            "Graph inputs differ");
        assert_eq!(left.outputs, right.outputs,
            "Graph outputs differ");
        ...
        for (l, r) in left.nodes.iter().zip(right.nodes.iter()) {
            ...
            assert_eq!(format!("{:?}", l.op), format!("{:?}", r.op),
                "SirOp mismatch for node {:?}: {:?} vs {:?}", l.id, l.op, r.op);
            assert_eq!(format!("{:?}", l.metadata), format!("{:?}", r.metadata),
                "SirMetadata mismatch for node {:?}", l.id);
        }
    }
    ...
    fn test_single_const_node() {
        let graph = SirGraph {
            nodes: vec![make_node("const_0", SirOp::Const {
    ...
    fn test_single_fill_node() {
        let graph = SirGraph {
            nodes: vec![make_node("fill_0", SirOp::Fill {
                shape: vec![1, 512, 896],
                value: 0.0,
                dtype: MilDtype::Fp16,
            })],
    ...
    fn test_single_identity_placeholder() {
        let graph = SirGraph {
            nodes: vec![make_node("input_0", SirOp::Identity {
    ...
    fn test_linear_projection_chain() {
        ...
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity {
                    input: SirNodeId("__placeholder__".to_string()),
                }),
                make_node("linear_0", SirOp::LinearProjection {
    ...
    fn test_rope_transform_node() {
        let graph = SirGraph {
            nodes: vec![
                ...
                make_node("rope_0", SirOp::RoPETransform {
                    input: SirNodeId("input".to_string()),
    ...
    fn test_decode_step_minimal() {
        let graph = SirGraph {
            nodes: vec![
                make_node("token", SirOp::Identity {
                    input: SirNodeId("__placeholder__".to_string()),
                }),
                make_node("decode_0", SirOp::DecodeStep {
    ...
    fn test_sdpa_without_mask() {
        let graph = SirGraph {
            nodes: vec![
                ...
                make_node("sdpa", SirOp::ScaledDotProductAttention {
            ...
            inputs: vec![SirNodeId("q".to_string()), SirNodeId("k".to_string()), SirNodeId("v".to_string())],
    ...
    fn test_sdpa_with_mask_and_scale() {
        let graph = SirGraph {
            nodes: vec![
                ...
                make_node("k", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("v", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("mask", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("sdpa", SirOp::ScaledDotProductAttention {
                    query: SirNodeId("q".to_string()),
                    key: SirNodeId("k".to_string()),
                    value: SirNodeId("v".to_string()),
    ...
    fn test_attention_block() {
        let graph = SirGraph {
            nodes: vec![
                ...
                make_node("k", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("v", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("attn", SirOp::AttentionBlock {
    ...
    fn test_state_read() {
        let graph = SirGraph {
            nodes: vec![make_node("k_cache", SirOp::StateRead {
    ...
    fn test_state_write() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("write_k", SirOp::StateWrite {
                    state_id: "kv_cache_k_0".to_string(),
                    offset: 0,
    ...
    fn test_unary_elementwise_ops() {
        ...
        let ops: Vec<(&str, SirOp)> = vec![
            ...
            ("gelu_0", SirOp::Gelu { input: SirNodeId("input".to_string()), mode: "exact".to_string() }),
        ...
        let mut nodes = vec![make_node("input", SirOp::Identity {
            input: SirNodeId("__placeholder__".to_string()),
        })];
    ...
    fn test_binary_elementwise_ops() {
        let ops: Vec<(&str, SirOp)> = vec![
            ("add_0", SirOp::Add { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("mul_0", SirOp::Mul { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("sub_0", SirOp::Sub { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("max_0", SirOp::Maximum { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("min_0", SirOp::Minimum { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("div_0", SirOp::RealDiv { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("pow_0", SirOp::Pow { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("eq_0", SirOp::Equal { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("ne_0", SirOp::NotEqual { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("gt_0", SirOp::Greater { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("ge_0", SirOp::GreaterEqual { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("lt_0", SirOp::Less { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
            ("le_0", SirOp::LessEqual { x: SirNodeId("input".to_string()), y: SirNodeId("input".to_string()) }),
        ];
    ...
    fn test_reduction_ops() {
        let ops: Vec<(&str, SirOp)> = vec![
            ("sum_0", SirOp::ReduceSum { input: SirNodeId("input".to_string()), axes: vec![2], keep_dims: true }),
            ("mean_0", SirOp::ReduceMean { input: SirNodeId("input".to_string()), axes: vec![2], keep_dims: false }),
            ("max_0", SirOp::ReduceMax { input: SirNodeId("input".to_string()), axes: vec![1, 2], keep_dims: true }),
            ("min_0", SirOp::ReduceMin { input: SirNodeId("input".to_string()), axes: vec![2], keep_dims: false }),
            ("prod_0", SirOp::ReduceProd { input: SirNodeId("input".to_string()), axes: vec![2], keep_dims: false }),
            ("argmax_0", SirOp::ReduceArgmax { input: SirNodeId("input".to_string()), axis: 2, keep_dims: false }),
            ("argmin_0", SirOp::ReduceArgmin { input: SirNodeId("input".to_string()), axis: 1, keep_dims: true }),
        ];
    ...
    fn test_reshape_and_transpose() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("reshape_0", SirOp::Reshape {
                    input: SirNodeId("input".to_string()),
                    target_shape: vec![1, 14, 512, 64],
                }),
                make_node("transpose_0", SirOp::Transpose {
    ...
    fn test_concat_split() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("split_0", SirOp::Split {
                    input: SirNodeId("input".to_string()),
                    axis: 2,
                    num_splits: 2,
                }),
                make_node("concat_0", SirOp::Concat {
                    inputs: vec![SirNodeId("split_0".to_string())],
                    axis: 2,
                }),
            ],
            inputs: vec![SirNodeId("input".to_string())],
    ...
    fn test_expand_dims_squeeze() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("expand_0", SirOp::ExpandDims {
                    input: SirNodeId("input".to_string()),
                    axis: vec![1],
                }),
                make_node("squeeze_0", SirOp::Squeeze {
                    input: SirNodeId("expand_0".to_string()),
                    axis: vec![1],
                }),
            ],
            inputs: vec![SirNodeId("input".to_string())],
    ...
    fn test_tile_and_pad() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("tile_0", SirOp::Tile {
                    input: SirNodeId("input".to_string()),
                    reps: vec![1, 1, 4, 1],
                }),
                make_node("pad_0", SirOp::Pad {
    ...
    fn test_gather() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("indices", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("gather_0", SirOp::Gather {
                    input: SirNodeId("input".to_string()),
    ...
    fn test_matmul() {
        let graph = SirGraph {
            nodes: vec![
                ...
                make_node("mm_0", SirOp::MatMul {
                    a: SirNodeId("a".to_string()),
                    b: SirNodeId("b".to_string()),
                }),
            ],
    ...
    fn test_select_op() {
        let graph = SirGraph {
            nodes: vec![
                make_node("cond", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                ...
                make_node("select_0", SirOp::Select {
                    condition: SirNodeId("cond".to_string()),
                    x: SirNodeId("x".to_string()),
                    y: SirNodeId("y".to_string()),
                }),
            ],
            inputs: vec![SirNodeId("cond".to_string()), SirNodeId("x".to_string()), SirNodeId("y".to_string())],
    ...
    fn test_softmax() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("softmax_0", SirOp::Softmax {
                    input: SirNodeId("input".to_string()),
                    axis: -1,
                }),
            ],
    ...
    fn test_conv_op() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("weight", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("conv_0", SirOp::Conv {
                    input: SirNodeId("input".to_string()),
    ...
    fn test_slice_by_index() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("slice_0", SirOp::SliceByIndex {
                    input: SirNodeId("input".to_string()),
                    begin: vec![0, 0, 0, 0],
                    end: vec![-1, -1, -1, 64],
                    stride: vec![1, 1, 1, 1],
    ...
    fn test_quantize_dequantize() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("quant_0", SirOp::Quantize {
                    input: SirNodeId("input".to_string()),
                    scale: 0.1,
                    zero_point: 0,
                    axis: -1,
                ...
                make_node("dequant_0", SirOp::Dequantize {
    ...
    fn test_constexpr_ops() {
        let graph = SirGraph {
            nodes: vec![
                make_node("affine_0", SirOp::ConstexprAffineDequantize {
                    quantized_data: "weight_q.bin".to_string(),
                    scale: 0.05,
                    zero_point: 128,
                    axis: 0,
                }),
                make_node("lut_0", SirOp::ConstexprLutToDense {
                    indices: "palette_idx.bin".to_string(),
                    lut: "palette_lut.bin".to_string(),
                    num_bits: 4,
                }),
                make_node("sparse_0", SirOp::ConstexprSparseToDense {
                    nonzero_data: "sparse_data.bin".to_string(),
                    shape: vec![256, 512],
                    default_value: 0.0,
                }),
                make_node("cast_0", SirOp::ConstexprCast {
    ...
    fn test_sampler_op() {
        let graph = SirGraph {
            nodes: vec![
                make_node("logits", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("sampler_0", SirOp::Sampler {
    ...
    fn test_layer_norm_and_batch_norm() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("ln_0", SirOp::LayerNorm {
                    input: SirNodeId("input".to_string()),
                ...
                make_node("bn_0", SirOp::BatchNorm {
    ...
    fn test_realistic_decode_pipeline() {
        ...
        let graph = SirGraph {
            nodes: vec![
                make_node("token", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("k_cache", SirOp::StateRead {
                    state_id: "kv_k_0".to_string(),
                    offset: 0,
                    shape: vec![1, 14, 512, 64],
                }),
                make_node("v_cache", SirOp::StateRead {
                    state_id: "kv_v_0".to_string(),
                    offset: 0,
                    shape: vec![1, 14, 512, 64],
                }),
                make_node("q_proj", SirOp::LinearProjection {
                ...
                make_node("o_proj", SirOp::LinearProjection {
                    input: SirNodeId("sdpa".to_string()),
                    weight: "o_proj.weight".to_string(),
                    bias: None,
                }),
                make_node("rms_norm", SirOp::RMSNorm {
    ...
    fn test_graph_io_preserved_multiple_inputs() {
        let graph = SirGraph {
            nodes: vec![
                make_node("in_a", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("in_b", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("add", SirOp::Add {
    ...
    fn test_graph_io_preserved_empty_inputs() {
        let graph = SirGraph {
            nodes: vec![make_node("const_0", SirOp::Fill {
                shape: vec![1, 128],
                value: 1.0,
                dtype: MilDtype::Fp16,
            })],
    ...
    fn test_run_returns_ok() {
        ...
        let graph = SirGraph {
            nodes: vec![make_node("input", SirOp::Identity {
                input: SirNodeId("__placeholder__".to_string()),
            })],
            inputs: vec![SirNodeId("input".to_string())],
    ...
    fn test_run_result_unwraps_cleanly() {
        ...
        let graph = SirGraph {
            nodes: vec![make_node("node_0", SirOp::Const {
    ...
    fn test_idempotent_multi_node() {
        let graph = SirGraph {
            nodes: vec![
                ...
                make_node("sdpa_0", SirOp::ScaledDotProductAttention {
    ...
    fn test_large_graph_many_nodes() {
        ...
        for i in 0..50 {
            ...
            nodes.push(make_node(&format!("relu_{}", i), SirOp::Relu {
                input: SirNodeId(prev),
            }));
        }
    ...
    fn test_pooling_ops() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("maxpool_0", SirOp::MaxPool {
                    input: SirNodeId("input".to_string()),
                ...
                make_node("avgpool_0", SirOp::AvgPool {
    ...
    fn test_recurrent_ops() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("h0", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("rnn_0", SirOp::Rnn {
                    input: SirNodeId("input".to_string()),
    ...
    fn test_lstm_op() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("h0", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("c0", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("lstm_0", SirOp::Lstm {
                    input: SirNodeId("input".to_string()),
                    initial_h: SirNodeId("h0".to_string()),
            ...
            inputs: vec![SirNodeId("input".to_string()), SirNodeId("h0".to_string()), SirNodeId("c0".to_string())],
    ...
    fn test_cond_op() {
        let graph = SirGraph {
            nodes: vec![
                make_node("pred", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("cond_0", SirOp::Cond {
    ...
    fn test_while_loop_op() {
        let graph = SirGraph {
            nodes: vec![
                make_node("var_0", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("while_0", SirOp::WhileLoop {
    ...
    fn test_random_ops() {
        let graph = SirGraph {
            nodes: vec![
                make_node("bernoulli_0", SirOp::RandomBernoulli {
                    shape: vec![1, 128],
                    prob: 0.5,
                    seed: Some(42),
                    dtype: MilDtype::Fp16,
                }),
                make_node("normal_0", SirOp::RandomNormal {
                    shape: vec![1, 128],
                    mean: 0.0,
                    stddev: 1.0,
                    seed: None,
                ...
                make_node("uniform_0", SirOp::RandomUniform {
                    shape: vec![1, 64],
                    low: -1.0,
                    high: 1.0,
                    seed: Some(123),
    ...
    fn test_conv_transpose() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("weight", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("conv_t_0", SirOp::ConvTranspose {
                    input: SirNodeId("input".to_string()),
                    weight: SirNodeId("weight".to_string()),
                    pad_type: "custom".to_string(),
                    groups: 1,
    ...
    fn test_reshape_like_and_flatten() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("ref", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("reshape_like_0", SirOp::ReshapeLike {
                    input: SirNodeId("input".to_string()),
                    ref_tensor: SirNodeId("ref".to_string()),
                }),
                make_node("flatten_0", SirOp::Flatten2d {
                    input: SirNodeId("reshape_like_0".to_string()),
                    axis: 1,
                }),
            ],
    ...
    fn test_reverse_op() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("reverse_0", SirOp::Reverse {
                    input: SirNodeId("input".to_string()),
                    axes: vec![1],
                }),
            ],
    ...
    fn test_space_depth_ops() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("d2s_0", SirOp::DepthToSpace {
                    input: SirNodeId("input".to_string()),
                    block_size: 2,
                }),
                make_node("s2d_0", SirOp::SpaceToDepth {
                    input: SirNodeId("d2s_0".to_string()),
                    block_size: 2,
                }),
                make_node("shuffle_0", SirOp::PixelShuffle {
                    input: SirNodeId("s2d_0".to_string()),
                    upscale_factor: 2,
                }),
                make_node("unshuffle_0", SirOp::PixelUnshuffle {
    ...
    fn test_misc_tensor_ops() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("cumsum_0", SirOp::Cumsum {
                    input: SirNodeId("input".to_string()),
                    axis: 1,
                    exclusive: false,
                    reverse: false,
                }),
                make_node("fill_like_0", SirOp::FillLike {
                    ref_tensor: SirNodeId("cumsum_0".to_string()),
                    value: 0.0,
                    dtype: MilDtype::Fp16,
                }),
                make_node("range_0", SirOp::Range1d {
                    start: 0.0,
                    end: 512.0,
                    step: 1.0,
                }),
    ...
    fn test_parametric_activations() {
        let graph = SirGraph {
            nodes: vec![
                make_node("input", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("leaky_0", SirOp::LeakyRelu { input: SirNodeId("input".to_string()), alpha: 0.01 }),
                make_node("scaled_tanh_0", SirOp::ScaledTanh { input: SirNodeId("leaky_0".to_string()), alpha: 1.0, beta: 0.5 }),
                make_node("thresh_0", SirOp::ThresholdedRelu { input: SirNodeId("scaled_tanh_0".to_string()), alpha: 1.0 }),
            ],
            inputs: vec![SirNodeId("input".to_string())],
    ...
    fn test_einsum_op() {
        let graph = SirGraph {
            nodes: vec![
                make_node("a", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("b", SirOp::Identity { input: SirNodeId("__placeholder__".to_string()) }),
                make_node("einsum_0", SirOp::Einsum {
                    inputs: vec![SirNodeId("a".to_string()), SirNodeId("b".to_string())],
    ...
    fn test_constexpr_blockwise_shift_scale() {
        let graph = SirGraph {
            nodes: vec![make_node("bwss_0", SirOp::ConstexprBlockwiseShiftScale {
