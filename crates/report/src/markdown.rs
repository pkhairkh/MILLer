...
impl MarkdownReporter {
    ...
    pub fn format_compilation_report(
        ...
        // Title
        let model_id = manifest.get("model_id").and_then(|v| v.as_str()).unwrap_or("unknown");
        ...
        lines.push(String::new());
        let status = manifest.get("bridge_status").and_then(|v| v.as_str()).unwrap_or("unknown");
        ...
        if let Some(packages) = manifest.get("packages").and_then(|v| v.as_array()) {
            if packages.is_empty() {
                ...
                for pkg in packages {
                    ...
                    let role = pkg.get("role").and_then(|v| v.as_str()).unwrap_or("unknown");
                    let path = pkg.get("path").and_then(|v| v.as_str()).unwrap_or("N/A");
                    ...
                    if let Some(functions) = pkg.get("functions").and_then(|v| v.as_array()) {
                        ...
                        for func in functions {
                            let fname =
                                func.get("name").and_then(|v| v.as_str()).unwrap_or("unnamed");
                            let stateful =
                                func.get("stateful").and_then(|v| v.as_bool()).unwrap_or(false);
                            ...
                            if let Some(inputs) = func.get("inputs").and_then(|v| v.as_array()) {
                                for input in inputs {
                                    ...
                                    let dtype =
                                        input.get("dtype").and_then(|v| v.as_str()).unwrap_or("?");
                                    let shape = input
                                        .get("shape")
                            ...
                            if let Some(outputs) = func.get("outputs").and_then(|v| v.as_array()) {
                                for output in outputs {
                                    let oname =
                                        output.get("name").and_then(|v| v.as_str()).unwrap_or("?");
                                    let dtype =
                                        output.get("dtype").and_then(|v| v.as_str()).unwrap_or("?");
                                    let shape = output
                                        .get("shape")
                                        .and_then(|v| v.as_array())
                                        .map(|arr| {
        ...
        if let Some(br) = bridge_result {
            ...
            if let Some(files) = br.get("package_files").and_then(|v| v.as_array()) {
                ...
                for file in files {
                    ...
                    let fsize = file.get("size_bytes").and_then(|v| v.as_u64()).unwrap_or(0);
            ...
            if let Some(cp) = br.get("compute_plan") {
                let available = cp.get("available").and_then(|v| v.as_bool()).unwrap_or(false);
                lines.push(format!(
                    "**Compute Plan**: {}",
        ...
        lines.push(String::new());
        lines.push(
            "- Device-specific ANE placement not verified (requires Apple hardware)".to_string(),
        );
        lines.push(
            "- Numerical drift not measured (requires Apple hardware for predict())".to_string(),
        );
        lines.push(
    ...
    pub fn format_knowledge_report(&self, knowledge_update: &Value) -> String {
        ...
        let task_name =
            knowledge_update.get("task_name").and_then(|v| v.as_str()).unwrap_or("unknown");
        ...
        if let Some(observations) = knowledge_update.get("observations").and_then(|v| v.as_array())
        {
            ...
            for (i, obs) in observations.iter().enumerate() {
                let ktype = obs.get("knowledge_type").and_then(|v| v.as_str()).unwrap_or("Unknown");
                let op_pattern = obs.get("op_pattern").and_then(|v| v.as_str()).unwrap_or("?");
                let ane_legal = obs.get("ane_legal").and_then(|v| v.as_bool()).unwrap_or(false);
                let confidence = obs.get("confidence").and_then(|v| v.as_f64()).unwrap_or(0.0);
                let evidence_source =
                    obs.get("evidence_source").and_then(|v| v.as_str()).unwrap_or("?");
                let evidence_count =
                    obs.get("evidence_count").and_then(|v| v.as_u64()).unwrap_or(0);
        ...
        if let Some(comp_result) = knowledge_update.get("compilation_result") {
            ...
            lines.push(String::new());
            let status = comp_result.get("status").and_then(|v| v.as_str()).unwrap_or("unknown");
            let mlpackage =
                comp_result.get("mlpackage_produced").and_then(|v| v.as_bool()).unwrap_or(false);
    ...
    pub fn generate_diagnostics_report(&self, error_data: &Value, output_path: &str) -> Result<()> {
    ...
    pub fn format_diagnostics_report(&self, error_data: &Value) -> String {
        ...
        let status = error_data.get("status").and_then(|v| v.as_str()).unwrap_or("unknown");
        lines.push(format!("**Status**: {}", status));
