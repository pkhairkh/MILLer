# MILLer — Prioritised Action List

> Ranked by **impact × urgency**. Each task references findings in `docs/audit/tabula-rasa-v3.md` (§VII) and `ISSUES.md`.
> Estimates assume a single experienced Rust/Python developer.
> Generated by TABULA RASA v3 audit on 2026-05-04.
> Previous tasks T-01 through T-46 are all resolved — see CHANGELOG.md for details.
> Tasks T-47 through T-57 from v2 audit are all resolved — see CHANGELOG.md for details.
> Tasks T-58 through T-66 remain open from v2 audit.

---

## 🔴 CRITICAL — Do These Now

### T-67 · Fix CPU_ONLY_OPS Name Mismatches (T-49 Regression)

- **ISSUES ref**: I-41, I-42
- **AUDIT ref**: §II-A, §IV (B-13)
- **Severity**: CRITICAL
- **Effort**: S (0.5 day)
- **Detail**: Five CPU_ONLY_OPS entries from T-49 used MIL builder function names instead of `mil_op_name()` return values. Most critically, `"negative"` should be `"neg"` — MILNeg passes the CPU-only gate and is classified as ANE-legal despite having no ANEC converter. Additionally, `"reverse_square_root"` incorrectly marks rsqrt as CPU-only (it IS ANE-legal via `anec.r_sqrt`), `"rint"` should be `"round"`, and `"reciprocal"`/`"signbit"` are dead code (no MirOp variant produces those names). Fix: (1) Add `"neg"`, remove `"negative"`. (2) Remove `"reverse_square_root"`. (3) Replace `"rint"` with `"round"`. (4) Remove `"reciprocal"` and `"signbit"`. (5) Add test verifying every CPU_ONLY_OPS entry matches at least one `mil_op_name()`. (6) Move MILNeg from `Some(AneEngine::PE)` to `None` in `default_engine()`.

---

## 🟠 HIGH — Do These Next Sprint

### T-68 · Fix `extract_whdc()` NCHW Dimensional Swap

- **ISSUES ref**: I-43
- **AUDIT ref**: §II-D, §IV (B-14)
- **Severity**: HIGH
- **Effort**: S (0.5 day)
- **Detail**: `extract_whdc()` treats rank-4 shapes as CDHW `[channels, depth, height, width]` but Core ML MIL uses NCHW `[batch, channels, height, width]`. For `[1, 64, 128, 128]`: current returns `(w=128, h=128, d=64, c=1)` — channels and depth are swapped, so `max_tensor_channels` is checked against batch (always 1) and `max_tensor_depth` against channels. Fix: For rank-4 NCHW: `(shape[3], shape[2], 1, shape[1])`. For rank-3 CHW: `(shape[2], shape[1], 1, shape[0])`. Add unit tests.

### T-69 · Wire Pooling Kernel Size Validation

- **ISSUES ref**: I-44
- **AUDIT ref**: §II-B, §IV (B-15)
- **Severity**: HIGH
- **Effort**: S (0.5 day)
- **Detail**: `validate_pooling_constraints()` takes `kernel_size` but discards it with `let _ = kernel_size;` (same pattern as I-36, fixed in T-62). ANE constraint docs specify per-family pooling kernel limits (`pe_max_pooling_kh`, `pe_max_pooling_kw`). Fix: Add kernel_size validation against revision-specific HW limits.

### T-70 · Fix K/V Projection Alias Map Drop

- **ISSUES ref**: I-45
- **AUDIT ref**: §III (CQ-16), §IV (B-17)
- **Severity**: HIGH
- **Effort**: S (0.5 day)
- **Detail**: `build_input_alias_map` resolves `k_proj_pattern()` and `v_proj_pattern()` but discards them with `let _ = (k_proj, v_proj)`. For GQA models with separate K/V projections, K/V tensor references are never properly wired. Currently ALL Q/K/V split aliases point to the Q projection node. Fix: Use k_proj/v_proj patterns to build separate K/V alias entries. Add tests verifying alias map contents for GQA models.

### T-71 · Fix `Float64.element_size()` Returning 4 Instead of 8

- **ISSUES ref**: I-46
- **AUDIT ref**: §IV (B-16)
- **Severity**: HIGH
- **Effort**: S (0.5 day)
- **Detail**: `CoreMlDataType::Float32 | CoreMlDataType::Float64 => 4` — Float64 should be 8 bytes. This produces weight entries with half the required bytes for Float64 constants. Fix: Split the match arm: `Float32 => 4, Float64 => 8`. Add test.

### T-72 · Fix Palettize Qwen3 Name Heuristics

- **ISSUES ref**: I-47
- **AUDIT ref**: §II-E
- **Severity**: HIGH
- **Effort**: M (1 day)
- **Detail**: `palettize_weights` uses hardcoded Qwen3/LLaMA name patterns (`q_proj`, `k_proj`, `v_proj`, `o_proj`, `qkv`) to classify weights as attention vs MLP. Other architectures (GPT-2, T5, BART) won't match, getting wrong bit-width assignments. Fix: Use `ModelArchitecture` enum methods instead of hardcoded name patterns.

### T-73 · Fix `LM_HEAD_SHARD_SIZE` Hardcoding

- **ISSUES ref**: I-48
- **AUDIT ref**: §III (CQ-17), §IV (B-19)
- **Severity**: HIGH
- **Effort**: S (0.5 day)
- **Detail**: `LM_HEAD_SHARD_SIZE = 19000` is specific to Qwen3-0.6B (vocab_size=151936). Other models get wrong shard sizes. Same pattern as I-30/I-31. Fix: Derive from model vocab_size or ANE HW limits. At minimum, make it a parameter.

### T-74 · Fix `resolve_shard` FP16-Only Byte Offsets

- **ISSUES ref**: I-49
- **AUDIT ref**: §III (CQ-18), §IV (B-18)
- **Severity**: HIGH
- **Effort**: S (0.5 day)
- **Detail**: `resolve_shard` uses `hidden_size * 2` assuming FP16. F32 weights would be 4 bytes, INT8 would be 1 byte. Wrong byte offsets silently slice incorrect portions of weight data. Fix: Use actual dtype element_size from the tensor's metadata.

### T-58 · Add Tests for ir::payload, ir::shard_desc, ir::serialize

- **ISSUES ref**: I-32
- **AUDIT ref**: §V
- **Severity**: HIGH
- **Effort**: L (3 days)
- **Detail**: Three modules with 0% test coverage and 30 pub fn total. `payload.rs` has 16 untested pub fn (all 5 family payload types, `from_spec`, `from_spec_with_override`, `to_json`). `shard_desc.rs` has 6 untested pub fn (`sharded_pipeline_shards`, `lower_shard_to_mir`, `build_sharded_pipeline_pir`). `serialize.rs` has 8 untested pub fn (round-trip for SIR/AIR/MIR/PIR). The precision adaptation pipeline (`from_spec_with_override`) is completely untested end-to-end.

### T-59 · Add Tests for lab::session, lab::harness, lab::fallback

- **ISSUES ref**: I-33
- **AUDIT ref**: §V
- **Severity**: HIGH
- **Effort**: L (2 days)
- **Detail**: Three 0%-coverage critical modules. `session.rs` has 7 untested pub fn (task hashing, knowledge update, artifact manifest). `harness.rs` has 14 untested pub fn (LabRunBuilder, LabRun, all builder paths). `fallback.rs` has 3 untested pub fn (FallbackDetector::detect_from_timing). These are the lab's main orchestration entry points.

---

## 🟡 MEDIUM — Do These Soon

### T-60 · Fix Tile Decomposition Placeholder Zeros

- **ISSUES ref**: I-34
- **AUDIT ref**: §IV (B-9)
- **Severity**: MEDIUM
- **Effort**: S (0.5 day)
- **Detail**: Tile decomposition in `legality_rewrite.rs:542-543` generates reshape_shape with `0` placeholders. `resolve_reshape_zeros()` uses batch=1 heuristic for multi-zero resolution, which is incorrect for general Tile patterns. Use ctx dimensions when available; document that ctx is required for correct Tile decomposition.

### T-61 · Add Cross-Validation Test for Python vs Rust Emission

- **ISSUES ref**: I-35
- **AUDIT ref**: §III (D-1, D-4)
- **Severity**: MEDIUM
- **Effort**: M (1 day)
- **Detail**: Python bridge (coremltools subprocess) and Rust proto-direct path exist independently with no cross-validation. Create a structural equivalence test that emits the same MIR graph through both paths and verifies identical MIL topology. Document which ops are supported by each path.

### T-64 · Centralize Palette Bit-Width Validation

- **ISSUES ref**: I-38
- **AUDIT ref**: §II-E (A-13)
- **Severity**: MEDIUM
- **Effort**: S (0.5 day)
- **Detail**: Palette bit-width validation {1,2,3,4,6,8} appears in 3 places: `lut_projection.rs:151`, `task_spec.rs:937`, and `sir.rs:1087` (doc comment). Create `ane_ir::ane_layout::validate_palette_bits()` and call from all sites.

### T-65 · Unify CPU-Only Classification

- **ISSUES ref**: I-39
- **AUDIT ref**: §III (E-1)
- **Severity**: MEDIUM
- **Effort**: M (1 day)
- **Detail**: CPU-only op classification is maintained in two places: `CPU_ONLY_OPS` HashSet in `cpu_only_ops.rs` and `default_engine() → None` branch in `mir.rs`. These can and do diverge (I-41/I-42). Derive the CPU-only set from `default_engine() == None` to maintain a single source of truth.

### T-66 · Add Remaining MirOpCompat Variants

- **ISSUES ref**: I-40
- **AUDIT ref**: §II-A (resolved ops note)
- **Severity**: MEDIUM
- **Effort**: M (2 days)
- **Detail**: Ops with real ANEC converters that still map to `MirOpCompat::Unsupported`: BatchNorm, InstanceNorm, L2Norm, MaxPool, AvgPool, L2Pool, Quantize, Dequantize, Resize/Resample variants, CropResize, DepthToSpace, SpaceToDepth, PixelShuffle, PixelUnshuffle, BatchToSpace, SpaceToBatch, and others. These have ANE hardware support but lack proto emission code.

### T-75 · Fix FFI `coreml_model_destroy` Unsoundness

- **ISSUES ref**: I-50
- **AUDIT ref**: §III (CQ-22)
- **Severity**: MEDIUM
- **Effort**: S (0.5 day)
- **Detail**: `Box::from_raw(inner)` on a handle that was never allocated with `Box::new(ModelHandleInner)`. Latent UB if `coreml_model_load` is ever implemented. Fix: Align allocation and deallocation contracts.

### T-76 · Add Tests for coreml-ffi::api Module

- **ISSUES ref**: I-51
- **AUDIT ref**: §V
- **Severity**: MEDIUM
- **Effort**: M (1 day)
- **Detail**: The `api.rs` module has 5 pub fn methods with zero test coverage. Add tests for error handling, result construction, and platform detection logic.

### T-77 · Enforce PythonBridge Timeout

- **ISSUES ref**: I-52
- **AUDIT ref**: §III (CQ-19)
- **Severity**: MEDIUM
- **Effort**: S (0.5 day)
- **Detail**: `timeout_secs` field exists (default 300) but is never used. `execute_raw_payload` calls `.output()` without timeout. A hung Python subprocess blocks the compiler indefinitely. Fix: Use spawn with timeout loop or `wait-timeout` crate.

### T-78 · Remove Dead-Code `compare_with_python_bridge` Stub

- **ISSUES ref**: I-53
- **AUDIT ref**: §III (CQ-20)
- **Severity**: MEDIUM
- **Effort**: S (0.5 day)
- **Detail**: `compare_with_python_bridge()` always returns `None` for all fields. Never called. Implement or remove.

### T-79 · Log Warning When SafetensorsResolver Is Empty

- **ISSUES ref**: I-54
- **AUDIT ref**: §III (CQ-24)
- **Severity**: MEDIUM
- **Effort**: S (0.5 day)
- **Detail**: `from_traced_graph` returns empty resolver without logging any warning. All weights become zero-filled placeholders. Fix: Add `log::warn!()` when all resolution strategies fail.

### T-80 · Fix Fill Op `input_names()` Returning Empty Vec

- **ISSUES ref**: I-55
- **AUDIT ref**: §III (CQ-23)
- **Severity**: MEDIUM
- **Effort**: S (0.5 day)
- **Detail**: `MirOpCompat::Fill { .. } => vec![]` — Fill's shape is an input in Core ML MIL. Fix: Return shape input name in `input_names()`.

---

## 🔵 LOW — Do These Eventually

### T-81 · Fix `compat_input_dtype` String Matching

- **ISSUES ref**: I-56
- **AUDIT ref**: §III (CQ-25)
- **Severity**: LOW
- **Effort**: S (0.5 day)
- **Detail**: Use MIR node's declared dtype instead of name-based heuristics for input_ids detection.

### T-82 · Remove Dead-Code `mir_node_to_compat`

- **ISSUES ref**: I-57
- **AUDIT ref**: §III (CQ-26)
- **Severity**: LOW
- **Effort**: S (0.5 day)
- **Detail**: Function marked `#[allow(dead_code)]` — shape-aware version is used instead. Remove or document why kept.

### T-83 · Add BF16→FP16 Edge-Case Tests

- **ISSUES ref**: I-58
- **AUDIT ref**: §III (CQ-27)
- **Severity**: LOW
- **Effort**: S (0.5 day)
- **Detail**: Add tests for NaN, Inf, subnormals, negative zero in BF16→FP16 conversion.

### T-84 · Replace `eprintln!` With `log::warn!`

- **ISSUES ref**: I-59
- **AUDIT ref**: §III (CQ-5)
- **Severity**: LOW
- **Effort**: S (0.5 day)
- **Detail**: Library code should use structured logging, not stderr writes.

### T-85 · Gate Deprecated `kv_cache_rewrite` Behind Feature Flag

- **ISSUES ref**: I-60
- **AUDIT ref**: §III (CQ-6)
- **Severity**: LOW
- **Effort**: S (0.5 day)
- **Detail**: Deprecated module still compiled. Gate behind feature flag or remove entirely.

---

## Task → Issue Cross-Reference

| Task | Issue(s) | Severity | Effort | Status |
|------|----------|----------|--------|--------|
| T-67 | I-41, I-42 | CRITICAL | S | ⬜ |
| T-68 | I-43 | HIGH | S | ⬜ |
| T-69 | I-44 | HIGH | S | ⬜ |
| T-70 | I-45 | HIGH | S | ⬜ |
| T-71 | I-46 | HIGH | S | ⬜ |
| T-72 | I-47 | HIGH | M | ⬜ |
| T-73 | I-48 | HIGH | S | ⬜ |
| T-74 | I-49 | HIGH | S | ⬜ |
| T-58 | I-32 | HIGH | L | ⬜ |
| T-59 | I-33 | HIGH | L | ⬜ |
| T-60 | I-34 | MEDIUM | S | ⬜ |
| T-61 | I-35 | MEDIUM | M | ⬜ |
| T-64 | I-38 | MEDIUM | S | ⬜ |
| T-65 | I-39 | MEDIUM | M | ⬜ |
| T-66 | I-40 | MEDIUM | M | ⬜ |
| T-75 | I-50 | MEDIUM | S | ⬜ |
| T-76 | I-51 | MEDIUM | M | ⬜ |
| T-77 | I-52 | MEDIUM | S | ⬜ |
| T-78 | I-53 | MEDIUM | S | ⬜ |
| T-79 | I-54 | MEDIUM | S | ⬜ |
| T-80 | I-55 | MEDIUM | S | ⬜ |
| T-81 | I-56 | LOW | S | ⬜ |
| T-82 | I-57 | LOW | S | ⬜ |
| T-83 | I-58 | LOW | S | ⬜ |
| T-84 | I-59 | LOW | S | ⬜ |
| T-85 | I-60 | LOW | S | ⬜ |

---

## Archived Tasks (All Resolved)

### v2 Audit (T-47 through T-66)

| Task | Issue | Severity | Status |
|------|-------|----------|--------|
| T-47 | I-21 | ~~CRITICAL~~ Fixed | ✅ |
| T-48 | I-22 | ~~CRITICAL~~ Fixed | ✅ |
| T-49 | I-23 | ~~HIGH~~ Fixed (regression: I-41/I-42) | ✅ |
| T-50 | ~~I-24~~ | RETRACTED | RETRACTED |
| T-51 | I-25 | ~~HIGH~~ Fixed | ✅ |
| T-52 | I-26 | ~~HIGH~~ Fixed | ✅ |
| T-53 | I-27 | ~~HIGH~~ Fixed | ✅ |
| T-54 | I-28 | ~~HIGH~~ MEDIUM → Fixed | ✅ |
| T-55 | ~~I-29~~ | RETRACTED | RETRACTED |
| T-56 | I-30 | ~~HIGH~~ Fixed | ✅ |
| T-57 | I-31 | ~~HIGH~~ Fixed | ✅ |
| T-62 | I-36 | ~~MEDIUM~~ Fixed | ✅ |
| T-63 | I-37 | ~~MEDIUM~~ Fixed | ✅ |

### v1 Audit (T-01 through T-46)

Tasks T-01 through T-46 from the v1 audit are all resolved. See `CHANGELOG.md` for details.
