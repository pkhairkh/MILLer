...
fn main() {
    let json_str = fs::read_to_string("/tmp/qwen3_trace.json")
    ...
    println!("hidden_size={}, num_heads={}, num_kv_heads={:?}, intermediate={}",
    ...
    println!("uses_rms_norm={}, uses_gqa={}, uses_rope={}",
        trace.model_config.uses_rms_norm,
        trace.model_config.uses_gqa,
        trace.model_config.uses_rope,
    );
    ...
    for node in sir.nodes.iter().take(15) {
        let desc = match &node.op {
            ...
            SirOp::Gather { input, indices, axis } => format!("Gather({},{},a={})", input.0, indices.0, axis),
