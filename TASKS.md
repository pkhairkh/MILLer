...
- Current host verification on this machine: `cargo test --workspace --quiet` passes with 501 passing tests and 1 ignored test; `python3 -m pytest python/tests/ -q` passes with 68 Python tests; `cargo clippy --all-targets --all-features -- -D warnings` passes cleanly; `cargo fmt --check --all` passes.
...
- Multi-function support is now real (Sprint 39): a two-function mlpackage (embedding + decode_step) is emitted and structurally verified on coremltools 9.0. Weight sharing between functions is implemented (Sprint 42) but coremltools 9.0 does NOT deduplicate constants across add_function() + ct.convert() boundaries — each function gets its own copy of the shared weight in weight.bin (though save_multifunction() does perform cross-function dedup). True weight sharing requires proto-direct manipulation or a future coremltools API.
...
- **Sprint 58 code quality improvements**: `MilDtypeRepr` has been unified into `MilDtype` (no more duplicate dtype enum); `ComputeUnits` has been unified into `ComputeUnitHint` (no more parallel compute-unit types); `ane-ir` now uses `anyhow::Result` instead of `Result<T, String>`; `MILTanhInverse` renamed to `MILAtanh` and `MLSinh` fixed to `MILSinh`; `CanonicalizePass` now validates SIR invariants (node ID uniqueness, no orphans); `StaticizePass` now detects dynamic shapes; `StateTopologyPass` removed from the pipeline. CI pipeline (`ci.yml`) and `rust-toolchain.toml` are configured. Python test infrastructure added with 68 tests across 3 test modules.
- **Sprint 59 ANE constraints grounding**: The compiler now has `AneFamily` (8 families), `AneRevision` (11 revisions), and `AneTarget` types with chip-to-family mapping. `AneEngine` enum (NE/PE/TransposeEngine) provides per-MirOp engine assignment. `AnePlacementValidator` checks hard ANE constraints (tensor rank, linear rank, SDPA constraints, CPU-only kill switch). Per-family op legality matrix seeded as knowledge. SDPA constraint validation added to AIR→MIR lowering. Palettization constraint knowledge seeded. The project can now make per-family legality decisions — previously it could not distinguish between ops that land on ANE for A14+ vs A11–A13.
- **Sprint 60 per-op constraints and CPU_ONLY hard gate**: Per-op constraint validation functions exist for conv, linear, gather, pooling, ArgMinMax, and tensor rank. Data type legality rules per ANE family are enforced (Int32/Fp64 rejection, quantization format validation, broadcast FP16-only on A11/A12). A `CPU_ONLY_OPS` set (108 entries) with reason codes is wired as a hard gate in `LegalityRewritePass` — CPU-only ops get confidence 0.0 regardless of soft scoring. `AneInterleave`/`AneLayout` types provide interleave and ChannelLast constraint validation. `AneHwLimits` provides per-revision hardware limit enforcement.
...
- This directly addresses the Sprint 42 finding: "coremltools 9.0's `add_function()` + `ct.convert()` does NOT deduplicate constants across function boundaries (though `save_multifunction()` does)." Proto-direct emission CAN deduplicate because Rust controls the weight.bin layout directly.
...
### Sprint 58 — Code Quality and Infrastructure Hygiene
**Status:** DONE (host-verified, 501 Rust tests + 68 Python tests passing)
**Residual:** CI pipeline is configured but not yet triggered on push (needs GitHub repository setup). Python tests use mocked coremltools — real integration tests require macOS. CanonicalizePass and StaticizePass have minimal implementations (validation-only) — full canonicalization and constant folding are future work.

### Sprint 59 — ANE Constraints Grounding
**Status:** DONE (host-verified, 501 Rust tests passing)
**Residual:** `AneFamily`/`AneRevision` types and chip-to-family mapping are derived from empirical ANE behavior — not verified against real hardware. `AnePlacementValidator` checks hard constraints but is not yet wired as a mandatory pre-pass before `LegalityRewritePass` in the CLI pipeline. `default_engine()` method on MirOp assigns engines for 167 variants but only 37 are currently reachable through the compiler — engine assignments for unimplemented ops are speculative. Per-family op legality matrix covers 94 ANE-landing ops and 53 CPU-only ops — the full 130+ CPU-only catalog from the constraint docs is partially represented (additional entries are in `cpu_only_ops_seed.json` from Sprint 60).
