...
mod tests {
    ...
    fn test_proto_output_name_linear() {
        let op =
            MirOp::MILLinear { name: "output".into(), x: nid("x"), weight: "w".into(), bias: None };
    ...
    fn test_proto_output_name_all_variants_have_name() {
        ...
        let cases: Vec<(MirOp, &str)> = vec![
            ...
            (
                MirOp::MILReshape { name: "reshape_out".into(), x: nid("x"), shape: vec![] },
                "reshape_out",
            ),
            (
    ...
    fn test_proto_input_refs_const_no_inputs() {
        let op =
            MirOp::MILConst { name: "c".into(), value_path: "vp".into(), dtype: MilDtype::Fp16 };
    ...
    fn test_proto_input_refs_linear() {
        let op = MirOp::MILLinear {
            name: "l".into(),
            x: nid("input_x"),
            weight: "weight_w".into(),
            bias: Some("bias_b".into()),
        };
    ...
    fn test_proto_input_refs_concat() {
        let op = MirOp::MILConcat {
            name: "c".into(),
            values: vec![nid("a"), nid("b"), nid("c_val")],
            axis: 0,
        };
    ...
    fn test_proto_input_refs_cond() {
        let op = MirOp::MILCond {
            name: "c".into(),
            pred: nid("flag"),
            true_graph: "true_fn".into(),
            false_graph: "false_fn".into(),
        };
    ...
    fn test_proto_input_refs_while_loop() {
        let op = MirOp::MILWhileLoop {
            name: "wl".into(),
            condition: "cond_fn".into(),
            body: "body_fn".into(),
            loop_vars: vec![nid("v1"), nid("v2")],
        };
    ...
    fn test_proto_input_refs_constexpr_lut_to_dense() {
        let op = MirOp::MILConstexprLutToDense {
            name: "cltd".into(),
            indices: "idx_weight".into(),
            lut: "lut_weight".into(),
            num_bits: 4,
        };
    ...
    fn test_proto_input_refs_gather() {
        let op =
            MirOp::MILGather { name: "g".into(), x: nid("data"), indices: nid("idx"), axis: 0 };
        assert_eq!(op.proto_input_refs(), vec!["data", "idx"]);
    }

    #[test]
    fn test_proto_input_refs_slice_update() {
        let op = MirOp::MILSliceUpdate {
            name: "su".into(),
            x: nid("data"),
            update: nid("upd"),
            begin: vec![0],
    ...
    fn test_proto_input_refs_select_where() {
        let select =
            MirOp::MILSelect { name: "s".into(), condition: nid("c"), x: nid("a"), y: nid("b") };
    ...
    fn test_is_proto_supported_constexpr_variants() {
        let op = MirOp::MILConstexprAffineDequantize {
            name: "cad".into(),
            quantized_data: "qd".into(),
            scale: 1.0,
            zero_point: 0,
            axis: 0,
        };
        assert!(op.is_proto_supported());
        let op2 = MirOp::MILConstexprLutToDense {
            name: "cltd".into(),
            indices: "i".into(),
            lut: "l".into(),
            num_bits: 4,
    ...
    fn test_is_proto_supported_random_unsupported() {
        let op = MirOp::MILRandomBernoulli {
            name: "rb".into(),
            shape: vec![1],
            prob: 0.5,
            seed: None,
    ...
    fn test_is_proto_supported_supported_ops_count() {
        ...
        assert!(
            supported_ops.iter().all(|op| op.is_proto_supported()),
    ...
    fn test_proto_input_refs_no_name_in_output() {
        ...
        let refs = op.proto_input_refs();
        assert!(
            !refs.contains(&"my_add_output".to_string()),
